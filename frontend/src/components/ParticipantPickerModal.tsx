...
interface Props {
  ...
  rowIndex: number;
  isDoubles: boolean;
  usedPlayerIds: number[]; // игроки уже заняты в этом турнире
  onSaved: () => void; // перезагрузить данные
}
...
export const ParticipantPickerModal: React.FC<Props> = ({ open, onClose, tournamentId, groupIndex, rowIndex, isDoubles, usedPlayerIds, onSaved }) => {
  ...
  const search = async (field: 'A' | 'B', q: string) => {
    ...
    try {
      ...
      if (field === 'A') setPlayersA(list); else setPlayersB(list);
    } catch (e) {
  ...
  const choose = (field: 'A' | 'B', p: Player) => {
    setError(null);
    if (disabledIds.has(p.id)) return; // уже участвует
    if (isDoubles) {
      const other = field === 'A' ? selectedB : selectedA;
      if (other && other.id === p.id) return; // нельзя выбрать одного и того же
    }
    if (field === 'A') {
      setSelectedA(p);
      setQueryA(`${p.last_name} ${p.first_name}`.trim());
      setPlayersA([]); // скрываем список
    } else {
      setSelectedB(p);
        ...
        <div className="p-4 flex flex-col gap-4">
          {!isDoubles ? (
            <div>
              <div className="flex items-center gap-2">
                <input value={queryA} onChange={(e) => { setQueryA(e.target.value); setActiveField('A'); search('A', e.target.value); }} placeholder="Поиск (фамилия, имя, отображаемое имя)" className="input" style={{ width: '100%' }} />
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input value={queryA} onChange={(e) => { setActiveField('A'); setQueryA(e.target.value); search('A', e.target.value); }} placeholder="Поиск (фамилия, имя, отображаемое имя)" className="input" style={{ width: '100%' }} />
              ...
              <div>
                ...
                <input value={queryB} onChange={(e) => { setActiveField('B'); setQueryB(e.target.value); search('B', e.target.value); }} placeholder="Поиск (фамилия, имя, отображаемое имя)" className="input" style={{ width: '100%' }} />
                <div className="max-h-60 overflow-auto border rounded mt-2">
                  {playersB.map(p => (
          ...
          <div className="border rounded p-3 flex flex-col gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              <input value={newLast} onChange={e => setNewLast(e.target.value)} placeholder="Фамилия" className="input" />
              <input value={newFirst} onChange={e => setNewFirst(e.target.value)} placeholder="Имя" className="input" />
              <button className="btn" onClick={addQuick} disabled={adding}>Добавить и выбрать</button>
            </div>
          </div>
