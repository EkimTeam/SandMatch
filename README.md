...
- Поддержка свободного формата (games_to=0) с возможностью ничьей

### Защита завершенных турниров
- Полная блокировка изменений для турниров со статусом "completed"
- Frontend и Backend проверки на всех endpoints
- Разрешено только удаление турнира и просмотр

### Экспорт и шаринг
- Экспорт PNG по кнопке «Поделиться»: снимается текущий вид таблиц/сетки
- Добавляются заголовок и логотип в шапке, внизу — футер «BeachPlay» слева и «скоро онлайн» справа
- Служебные элементы (переключатели, чекбокс фиксации, туры, нижняя панель) не попадают в картинку
