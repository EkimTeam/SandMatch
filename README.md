...
- Страница турнира: отрисовка групповых таблиц и «порядок игр», кнопки «Завершить», «Удалить», «Поделиться» (серый).
...
- Ввод счёта матча в матрице (MVP: один сет) через модальное окно.
  ...
  - Автоматическое обновление технических столбцов (победы, сеты, соотношения, геймы, место) после ввода счёта и при загрузке страницы.
...
- Улучшение ввода результатов матча: поддержка нескольких сетов и тай‑брейков согласно `SetFormat`.
- Сортировка и правила определения мест по пресету `Ruleset` (включая h2h при равенстве).
- Экспорт/шаринг результатов: печатная версия и PNG.
