...
Этот гайд описывает быстрый запуск проекта локально «как в продакшене», а также удобный режим разработки с Vite dev server.
...
## Запуск вариант A: «Как в продакшене» (без Vite dev server)

Цель — увидеть ту же картину, что и в продакшене, за счёт сборки фронтенда и подключения ассетов через `static/frontend/manifest.json`.
