...
2. Нажимается кнопка «Завершить турнир» → вызов `POST /api/tournaments/<id>/complete/`.
   - Расчёт рейтинга выполняется только если в настройках турнира включён флаг `is_rating_calc=true`.
   - Если `is_rating_calc=false`, турнир завершается без:
     - предложения присвоить стартовый рейтинг участникам без рейтинга;
     - автоматического присвоения стартовых рейтингов;
     - изменения рейтингов участников по итогам турнира.
   - Если `is_rating_calc=true`:
     - (опционально через UI) можно присвоить стартовые рейтинги участникам без рейтинга;
...
- Для стадии турнира (у которой есть `parent_tournament`) эффективное условие обсчёта:
  - `effective_is_rating_calc = stage.is_rating_calc AND master.is_rating_calc`.
- Если у головного (master) турнира `is_rating_calc=false`, обсчёт рейтинга не выполняется ни для одной стадии.
- Если у головного турнира `is_rating_calc=true`, но у конкретной стадии `is_rating_calc=false`, это допустимо:
  - головной турнир и другие стадии могут обсчитываться,
  - конкретная стадия не обсчитывается.
