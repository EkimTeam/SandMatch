# План разработки BeachPlay (MVP0 → MVP1)
...
- Плей‑офф: endpoint `POST /api/tournaments/{id}/match_save_score_full/` для сохранения всех сетов и автопродвижения победителя; интеграция в React UI (`KnockoutPage.tsx`).
- Список турниров: исправлен фильтр по системе проведения (включая «Кинг»).
- Рейтинг игроков:
  - Расчёт при завершении турнира (`POST /api/tournaments/{id}/complete/`), идемпотентность защищает от повторного пересчёта.
  - Стартовые рейтинги новых игроков по названию турнира (HARD=1100, MEDIUM=900, иначе 1000).
  - История и агрегаты: `PlayerRatingHistory` (поматчевые дельты) и `PlayerRatingDynamic` (по турниру), актуализирован график в карточке игрока.
  - Админ‑команды: `cleanup_tournament_rating`, `recompute_tournament_rating`.

### TODO/ограничения
- Чемпионский тай‑брейк: в агрегатах «Сеты» учитывать как 1:0/0:1 (сейчас учёт TB очками; требуется доработать отображение/агрегаты).
- Правила ранжирования перенести в конфигурируемый пресет `Ruleset` (часть логики сейчас на фронтенде для групп).
- Публичный шаринг‑URL и печатная версия экспорта.
- Экспорт результатов: html‑шаблон + html2canvas → PNG (уточнить макет печати).
...
Дополнения по рейтингу и олимпййке:
- `POST /api/tournaments/{id}/complete/` — завершение турнира с расчётом рейтинга, идемпотентно.
- Команды: `cleanup_tournament_rating`, `recompute_tournament_rating`.
- Плей‑офф: сохранение полного счёта всех сетов с автопродвижением.

## Ближайшие задачи

- Учёт решающего TB как 1:0/0:1 в агрегатах групп.
- Кнопка «Сгенерировать расписание» на странице турнира (без захода в админку).
- Сортировка/ранжирование по `Ruleset`, включая H2H при равенстве — бэкенд‑сервис + UI.
...
- Добавить отображение рейтинга в узлы сетки и списки участников Олимпийки.
