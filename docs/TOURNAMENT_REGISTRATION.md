...
#### Frontend (статус: ОСНОВНОЙ ФЛОУ РЕАЛИЗОВАН)
...
- [ ] Доработки по UX/мелким правкам могут добавляться по мере появления кейсов.

- [ ] **Доработать фронтенд веб-регистрации:**
  - [ ] добавить/скорректировать кнопки и формы под существующие API
  - [ ] отобразить статусы регистрации (основной/резерв/ищу пару) и партнёра
  - [ ] реализовать UX по инвайтам (если требуется на вебе)
  - [ ] добавить индикацию свободных мест и лимита участников
  - [ ] обработка ошибок и валидация форм

- [ ] **Создать/обновить компоненты:**
  - [ ] модальное окно регистрации с выбором режима (solo / с партнёром / ищу пару)
  - [ ] компонент поиска партнёра с автокомплитом
  - [ ] список участников с разделением на основной/резерв/ищут пару
  - [ ] карточки игроков с возможностью отправки инвайта
