...
- [x] **Унифицирована регистрация в мини-аппе:**
  - [x] Старый эндпоинт `POST /api/mini-app/tournaments/{id}/register/` переписан через `RegistrationService`
  - [x] Поддержка индивидуальных турниров (SINGLES) - вызывает `register_single()`
  - [x] Поддержка парных турниров (DOUBLES):
    - С напарником (`partner_id` указан) - вызывает `register_with_partner()`
    - Без напарника - вызывает `register_looking_for_partner()`
  - [x] Весь флоу идёт через `TournamentRegistration`, `TournamentEntry` не создаётся напрямую
  - [x] Все эндпоинты используют `RegistrationService`:
    - `register()` - универсальная регистрация
    - `register_single()` - для индивидуальных турниров
    - `register_looking_for_partner()` - режим "ищу пару"
    - `register_with_partner()` - регистрация с напарником
    - `send_pair_invitation()` - отправка приглашения
    - `accept_invitation()` - принятие приглашения
    - `decline_invitation()` - отклонение приглашения
    - `leave_pair()` - выход из пары
    - `cancel_registration()` - полная отмена регистрации

#### Frontend (статус: ГОТОВО ✅)

- [x] **UX мини-аппа полностью реализован:**
  - [x] Экран списка турниров с индикацией регистрации (`is_registered`)
  - [x] Экран турнира со статусом регистрации пользователя
  - [x] Списки участников с правильной сортировкой:
    - Основной состав: по алфавиту
    - Резервный список: по очереди регистрации
    - Ищут пару: по алфавиту
  - [x] Группировка пар в одну плитку (отображение "ФИО1 / ФИО2" с рейтингом пары)
  - [x] Кнопки для трёх сценариев регистрации (solo / ищу пару / с партнёром)
  - [x] **PartnerSearchModal** с рейтингами BP и рекомендациями по истории
  - [x] Кнопка "Пригласить" доступна для:
    - Игроков в статусе `looking_for_partner`
    - Незарегистрированных пользователей
  - [x] Удалён неиспользуемый метод `registerForTournament()` из API клиента
