...
- `GET /api/mini-app/tournaments/` — список турниров с индикацией регистрации
- `GET /api/mini-app/tournaments/{id}/` — детальный просмотр турнира
- `GET /api/mini-app/tournaments/my_tournaments/` — мои турниры

#### Эндпоинты регистрации (полностью унифицированы через RegistrationService)

**Просмотр состояния:**
- `GET /api/mini-app/tournaments/{id}/my-registration/` — моя регистрация на турнир
  - возвращает статус, напарника, команду
- `GET /api/mini-app/tournaments/{id}/participants/` — списки участников
  - основной состав (сортировка по алфавиту)
  - резервный список (сортировка по очереди регистрации)
  - ищут пару (сортировка по алфавиту)

**Регистрация:**
- `POST /api/mini-app/tournaments/{id}/register-single/` — одиночная регистрация (для индивидуальных турниров)
- `POST /api/mini-app/tournaments/{id}/register-looking-for-partner/` — регистрация в режиме "ищу пару"
- `POST /api/mini-app/tournaments/{id}/register-with-partner/` — регистрация с напарником
  - body: `{"partner_id": <player_id>}`
  - создаёт две синхронные регистрации для обоих игроков
  - отправляет уведомление напарнику, если у него есть Telegram

**Поиск напарника:**
- `GET /api/mini-app/tournaments/{id}/search-players/?q=<query>` — поиск игроков по ФИО
  - исключает текущего игрока из результатов
  - показывает рейтинг BP каждого игрока
  - помечает `is_registered=true` для игроков в основном/резервном списке
  - минимальная длина запроса: 2 символа
- `GET /api/mini-app/tournaments/{id}/recent-partners/` — рекомендации напарников
  - 3 последних уникальных напарника
  - дополнение до 5 из наиболее частых напарников
  - с рейтингами BP и статусом регистрации

**Приглашения:**
- `GET /api/mini-app/invitations/` — список входящих приглашений
- `POST /api/mini-app/tournaments/{id}/send-pair-invitation/` — отправить приглашение
  - body: `{"receiver_id": <player_id>, "message": "текст"}`
  - доступно только для игроков в статусе `looking_for_partner`
- `POST /api/mini-app/invitations/{id}/accept/` — принять приглашение
  - формирует пару через `RegistrationService.register_with_partner`
- `POST /api/mini-app/invitations/{id}/decline/` — отклонить приглашение

**Управление регистрацией:**
- `POST /api/mini-app/tournaments/{id}/leave-pair/` — выход из пары
  - оба игрока переводятся в статус `looking_for_partner`
  - используется `transaction.atomic()` и `select_for_update()` для предотвращения race conditions
  - сначала обнуляются `team` и `partner`, затем удаляется `TournamentEntry`
- `POST /api/mini-app/tournaments/{id}/cancel-registration/` — полная отмена регистрации
  - удаляет регистрацию текущего игрока
  - если есть напарник, переводит его в `looking_for_partner`

**Статус:** backend полностью унифицирован через `RegistrationService`. Все операции идут через `TournamentRegistration`.

### 5.3. Telegram-бот

**Файлы:** 
- `apps/telegram_bot/bot/handlers/tournaments.py` - базовые функции и команды
- `apps/telegram_bot/bot/handlers/registration.py` - обработчики регистрации

#### 5.3.1. Базовая инфраструктура (ГОТОВО ✅)

- Профиль и привязка `TelegramUser ↔ Player`
- Все операции регистрации идут через `RegistrationService` и `TournamentRegistration`
- Взаимодействие с ORM — через `@sync_to_async`

#### 5.3.2. Раздел «Турниры»

Используется командой `/tournaments` и callback‑кнопкой `cmd_tournaments`.

- Показываются до трёх блоков:
  - **🔥 Активные турниры** (`status=active`)
  - **📝 Турниры для регистрации** (`status=created`)
  - **✅ Завершённые турниры** (`status=completed`, с лимитом по количеству)
- Для каждого турнира текст формируется функцией:
  - `format_tournament_info(tournament, is_registered=False, place=None, winner=None)`
- Внизу выводится итоговая строка:
  - `Всего в истории турниров: N`, где `N = Tournament.objects.count()` (обёртка `get_total_tournaments_count`).

**Клавиатуры для турниров:**

- Ряд 1:
  - `📱 В мини-апп` → `WEB_APP_URL/mini-app/tournaments/{id}` (через `WebAppInfo`)
  - `🌐 На BeachPlay.ru` → `WEB_APP_URL/tournaments/{id}`
- Ряд 2:
  - `◀️ Назад` → возврат в главное меню (`callback_data="main_menu"`)

#### 5.3.3. Раздел «Мои турниры»

Callback‑кнопка `cmd_mytournaments`.

- Турниры пользователя берутся через `get_user_tournaments(player_id)`:
  - ищутся команды игрока (`Team.player_1 / Team.player_2`),
  - по ним — `TournamentEntry`,
  - далее — `Tournament` по `id__in`.
- Турниры разбиваются на три группы:
  - активные (`active`),
  - предстоящие (`created`),
  - завершённые (`completed`).
- Показ ограничен 5 последними турнирами по логике мини‑аппа (приоритет active/created, затем ограниченное число completed).

**Отображение мест и победителей:**

- Для завершённых турниров используется:
  - `get_user_place(tournament_id, player_id)` → возвращает строку `"N"` или `"N–M"` по `TournamentPlacement`;
  - `get_tournament_winner(tournament_id)` → победитель (как в разделе «Турниры»).
- В `format_tournament_info` передаются аргументы `is_registered=True, place=user_place, winner=winner`.
  - Если турнир завершён и `place` есть → добавляется строка `🏆 Твоё место N`.
  - Если завершён и `place` нет → строка `"✅ Ты зарегистрирован"` **не добавляется**.
  - Если передан `winner` → добавляется `🥇 Победитель: ...`.

Клавиатуры для плиток в «Мои турниры» такие же, как в разделе «Турниры» (ряд с мини‑апп/сайтом + `◀️ Назад`).

#### 5.3.4. Раздел «Мои заявки»

Callback‑кнопка `cmd_myregistration`.

- Берутся все турниры пользователя через `get_user_tournaments(player_id)`, затем фильтруются только турниры со статусом `created` (идёт набор).
- Для каждого такого турнира через `get_registration_status(tournament_id, player_id)` получаются:
  - статус регистрации (`main_list`, `reserve_list`, `looking_for_partner`, `invited`),
  - напарник (если есть),
  - команда и вспомогательные поля.
- В тексте плитки выводится:
  - название турнира,
  - текущий статус регистрации с иконкой,
  - напарник (ФИО полностью), если есть,
  - дата турнира (если указана).
- Клавиатура:
  - `📱 В мини-апп` / `🌐 На BeachPlay.ru` (как выше),
  - `❌ Отменить регистрацию`,
  - `◀️ Назад`.

#### 5.3.5. Регистрация и поиск напарника (без изменений по сравнению с предыдущей версией)

Сценарии регистрации (индивидуальные и парные турниры, FSM поиска напарника, отмена регистрации и выход из пары) остаются как описано выше в разделе 2 и предыдущей версии 5.3, и реализованы на базе `RegistrationService` и моделей `TournamentRegistration` / `PairInvitation`.
...
- [x] **MiniAppTournamentDetail** (`frontend/src/pages/MiniApp/MiniAppTournamentDetail.tsx`):
  - Детальная страница турнира с полной информацией
  - Динамическая кнопка внизу экрана:
    - "Зарегистрироваться" — для незарегистрированных
    - "Найти напарника" — для пользователей в статусе `looking_for_partner`
    - "Отказаться от турнира" — для зарегистрированных пользователей
  - Автоматическое обновление данных после регистрации (`loadTournament` + `loadMyRegistration`)
  - Кнопка "Отменить" рядом с надписью "Вы зарегистрированы" открывает модалку выбора

- [x] **RegistrationModal** (`frontend/src/components/MiniApp/RegistrationModal.tsx`):
  - Модалка выбора способа регистрации для парных турниров
  - Скрывает кнопку "Ищу пару" если пользователь уже в этом статусе (`currentStatus === 'looking_for_partner'`)
  - Три режима:
    - "Ищу пару" — регистрация в режиме поиска напарника
    - "С напарником" — открывает `PartnerSearchModal`
  - Для индивидуальных турниров показывает простую форму регистрации

- [x] **PartnerSearchModal** (`frontend/src/components/MiniApp/PartnerSearchModal.tsx`):
  - Поиск напарника по ФИО с минимальной длиной запроса 2 символа
  - Рекомендации по истории (3 последних + дополнение до 5 из частых)
  - Отображение рейтинга BP для каждого игрока
  - Индикация зарегистрированных игроков (`is_registered`)
  - Оптимизированное поле поиска для мобильных:
    - Уменьшенные отступы (`px-3`)
    - Размер шрифта `text-base` для видимости текста
    - Явный цвет текста `text-gray-900`
    - Кнопка "Найти" с `flex-shrink-0` и `whitespace-nowrap`

- [x] **CancelRegistrationModal** (`frontend/src/components/MiniApp/CancelRegistrationModal.tsx`):
  - Модалка с выбором действия при отказе от турнира
  - Две опции:
    - "Отказаться от текущей пары" — оба игрока переходят в `looking_for_partner`
    - "Покинуть турнир полностью" — удаление регистрации, напарник переходит в `looking_for_partner`
  - Опция "Отказаться от пары" показывается только если `hasPartner === true`

- [x] **TournamentParticipants** (`frontend/src/components/MiniApp/TournamentParticipants.tsx`):
  - Списки участников с правильной сортировкой:
    - Основной состав: по алфавиту
    - Резервный список: по очереди регистрации
    - Ищут пару: по алфавиту
  - Группировка пар в одну плитку (отображение "ФИО1 / ФИО2" с рейтингом пары)
  - Кнопка "Пригласить" для игроков в статусе `looking_for_partner` и незарегистрированных
  - Индикация текущего игрока в списках

- [x] **InvitationsModal** (`frontend/src/components/MiniApp/InvitationsModal.tsx`):
  - Список входящих приглашений в пару
  - Кнопки "Принять" и "Отклонить" для каждого приглашения
  - Автоматическое обновление после действий

**UX улучшения:**

- [x] Явное закрытие конфликтующих модалок (при открытии одной закрывается другая)
- [x] Автоматическое обновление `myRegistration` после успешной регистрации
- [x] Корректное отображение кнопок и модалок без необходимости ручного обновления страницы
- [x] Оптимизация для мобильных устройств (видимость текста, размеры кнопок)
- [x] Haptic feedback для всех действий регистрации
- [x] Информативные сообщения об успехе/ошибке операций

### 6.4. Telegram-бот (статус: ГОТОВО ✅)

- [x] Профиль и привязка `TelegramUser ↔ Player`
- [x] Уведомления через `apps/telegram_bot/tasks.py`
- [x] **Полный флоу регистрации через бот:**
  - [x] Обновлены функции проверки регистрации (`check_registration`, `get_user_tournaments`) для использования `TournamentRegistration`
  - [x] Регистрация на индивидуальные турниры в один клик через `RegistrationService.register_single()`
  - [x] Выбор режима регистрации для парных турниров (ищу пару / с напарником)
  - [x] Регистрация в режиме "Ищу пару" через `RegistrationService.register_looking_for_partner()`
  - [x] Поиск напарника по ФИО через FSM (Finite State Machine)
  - [x] Регистрация с напарником через `RegistrationService.register_with_partner()`
  - [x] Команда `/myregistration` для просмотра статуса регистрации
  - [x] Отмена регистрации с выбором опций (выйти из пары / покинуть турнир)
  - [x] Все операции используют `RegistrationService` для единообразия с веб и мини-аппом
