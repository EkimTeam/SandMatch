...
BP (Beach Play) - внутренняя рейтинговая система SandMatch. Используется для балансировки турниров и подбора соперников.

**Ключевые особенности:**
- Шкала от 1000 до 2000 (целые числа)
- Автоматический расчет на основе результатов матчей (ELO-подобная система)
- Расчет начального рейтинга из BTR
...
```
2000 - Элита (BTR 1500+)
1600-1999 - Профессионалы
1400-1599 - Продвинутые
1200-1399 - Средний уровень
1000-1199 - Начинающие
1000 - Новички (BTR < 80)
```

**Примеры соответствия BTR → BP (начальный рейтинг):**
```
BTR < 80   → BP 1000
BTR 100    → BP 1080
BTR 200    → BP 1480
BTR 300    → BP 1880 (кап на 1600 для BTR < 1500)
BTR 1500   → BP 1600
BTR 2000   → BP 1750
...
class Player(models.Model):
    current_rating = models.IntegerField(default=0)  # BP рейтинг (1000-2000)
    btr_player = models.OneToOneField('btr.BtrPlayer', null=True)  # Связь с BTR
```

**Хранение:** Рейтинг хранится как целое число от 1000 до 2000:
- Новичок: 1000
- Средний: 1300
- Профессионал: 1700
- Элита: 2000

**Отображение:** Рейтинг отображается как есть, без преобразований.
